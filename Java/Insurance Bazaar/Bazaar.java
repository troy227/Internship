...
public class Bazaar {
	...
	public List<Integer> searchBasedOnPolicyType(String policyType){
        ...
        list=getPolicyMap().entrySet().stream().filter(value -> value.getValue().contains(policyType)).map(Map.Entry::getKey).collect(Collectors.toList());
        Collections.sort(list);
		System.out.println("List is - "+list);
		System.out.println("Map is-"+getPolicyMap());
